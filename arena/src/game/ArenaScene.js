...
/**
 * ArenaScene — movement + Combat V1.
 *   Movement: WASD/arrows, W/↑/Space jump, S/↓ descend (real Diggerz scheme).
 *   Combat:   mouse aim, left-mouse use, mouse-wheel hotbar select.
 * Weapons are the confirmed Diggerz Fake Sword + Blue Ray Gun; combat numbers
 * are PROPOSED standalone values (CombatConfig).
 */
export class ArenaScene {
  constructor(canvas, { debugEl, hudEl, winEl, resetBtn, debugChk } = {}) {
    ...
    // P2 dummy: a still fighter on the floor with health + hurtbox.
    const restY = (this.map.rows - 1) * tileSize - 36;
    const dummyBody = MovementController.createBody(16 * tileSize, restY, 24, 36);
    dummyBody.facing = -1;
    this.dummy = { body: dummyBody };
  ...
  _aimAngle() {
    ...
    return computeAim(b.x + b.w / 2, b.y + b.h / 2, this.combatInput.aimX, this.combatInput.aimY);
  }
  ...
  _loop(now) {
    ...
    // Combat input (real Diggerz controls): wheel selects, left-mouse uses.
  ...
  _render() {
    ...
    for (let c = 0; c <= map.cols; c++) { ctx.beginPath(); ctx.moveTo(c * t, 0); ctx.lineTo(c * t, canvas.height); ctx.stroke(); }
    for (let r = 0; r <= map.rows; r++) { ctx.beginPath(); ctx.moveTo(0, r * t); ctx.lineTo(canvas.width, r * t); ctx.stroke(); }
    ...
    if (d.health.alive) {
      this.character.draw(ctx, this._bodyState(d.body), now, P2_PALETTE);
    ...
    for (const w of this.combat.hotbar.slots) {
      ...
      for (const p of this.combat.projectilesOf(w.id)) this._drawProjectile(p);
    }

    // Melee swing visual (sword) while active.
    for (const w of this.combat.hotbar.slots) {
      if (w.combat.kind !== 'melee') continue;
      const arc = this.combat.resolvers[w.id].debugArc(now, this.player.body);
      if (arc) this._drawSwing(arc);
    }
  ...
  _drawProjectile(p) {
    ...
    ctx.globalAlpha = 0.4; ctx.fillStyle = '#7fd1ff'; ctx.fillRect(-p.w, -1, p.w, 2);
    ctx.restore();
  }
  ...
  _drawCombatDebug() {
    ...
    for (const w of this.combat.hotbar.slots)
      if (w.combat.kind === 'projectile')
        for (const p of this.combat.projectilesOf(w.id)) stroke(p.aabb(), '#ffe27f');
  }
  ...
  _renderHotbar() {
    ...
    slots.forEach((w, i) => {
      ...
      if (selected) {
        ...
        ctx.fillText(w.name, this.canvas.width / 2, y - 6);
        ctx.textAlign = 'left';
      }
    });
  }
  ...
  _renderDebug() {
    ...
    this.debugEl.textContent = [
      `x ${f(s.x)}  y ${f(s.y)}`,
      `vx ${f(s.vx)}  vy ${f(s.vy)}`,
      `grounded ${s.grounded}  facing ${s.facing > 0 ? 'R' : 'L'}`,
      `anim ${s.anim}`,
      ``,
      `weapon   ${w.name} (id ${w.id})`,
      `P1 hp    ${a.hp}/${a.max}`,
      `P2 hp    ${d.hp}/${d.max} ${d.alive ? '' : '(dead)'}`,
